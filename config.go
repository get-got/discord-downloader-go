...
var (
	PLACEHOLDER_TOKEN    string = "REPLACE_WITH_YOUR_TOKEN"
	PLACEHOLDER_EMAIL    string = "REPLACE_WITH_YOUR_EMAIL"
	PLACEHOLDER_PASSWORD string = "REPLACE_WITH_YOUR_PASSWORD"
)
...
func createConfig() {
	...
	baseChannel := ConfigurationChannel{
		ChannelID:   "REPLACE_WITH_DISCORD_CHANNEL_ID_TO_DOWNLOAD_FROM",
...
func isCommandableChannel(m *discordgo.Message) bool {
	if isAdminChannelRegistered(m.ChannelID) {
		...
		if *channelConfig.AllowCommands || adminCheck(m) {
			return true
		}
	}
	return false
