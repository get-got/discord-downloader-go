...
func updateDiscordPresence() {
	if config.PresenceEnabled {
		...
		keys := [][]string{
			...
			{"{guilds}", fmt.Sprint(len(bot.State.Guilds))},
			{"{channels}", fmt.Sprint(len(config.Channels))},
			{"{adminChannels}", fmt.Sprint(len(config.AdminChannels))},
			{"{admins}", fmt.Sprint(len(config.Admins))},
			{"{timeShort}", timeShort},
			{"{timeLong}", timeLong},
			{"{timeShort24}", timeLastUpdated.Format("15:04")},
			{"{timeLong24}", timeLastUpdated.Format("15:04:05 MST - 1 January, 2006")},
